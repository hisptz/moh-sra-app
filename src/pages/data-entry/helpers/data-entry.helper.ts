...
export function onDataValueChange(
  ...
  if (element) {
    ...
    if (element && element.type && element.type === "checkbox") {
      elementValue = element.checked ? true : "not-applicable";
    } else if (element && element.type && element.type === "select-one") {
      elementValue = element.value ? element.value : "not-allowed";
    } else {
